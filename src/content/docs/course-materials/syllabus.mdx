...
the ability to re-attempt assignments for completion, tokens used for coursework may only be used in the event that a student
cannot submit a full assignment attempt on time. Using a token essentially "resets" the clock and gives the student `1`
additional week to complete a _first attempt_.
