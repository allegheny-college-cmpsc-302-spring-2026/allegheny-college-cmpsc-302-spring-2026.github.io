...
### Use of course tokens

Per the above department policy, students in a `300`-level course are allowed `2` tokens. Given that this course prices in
the ability to re-attempt assignments for completion, tokens are instead intended in the event that a student
cannot submit a full assignment attempt on time. Using a token essentially "resets" the timeline and gives the student `1`
additional week to complete any given attempt. Students _must use tokens up to a minimum of 24 hours before a given attempt's due
date_.
...
A student is unable to finish a _first_ attempt on an assignment. They use a token _before the deadline_ to add 
one week to the attempt "clock." After a few days, they submit the assignment attempt and are evaluated on a _first
attempt_ basis.

#### Token Example 2

A student would like to submit a _third_ attempt on an assignment, but needs more time to finish a time-intensive
detail. The student uses a token _before_ the deadline of the third attempt to extend the assignment due date and
is evaluated as a _third_ attempt.

#### Redeeming Tokens

To redeem a token, use the following form:

> <Icon name = "pen" class = "icon-prepend" /><a href = "https://forms.gle/9sHirVccAV3rEncf6">Assignment Attempt Form</a>
